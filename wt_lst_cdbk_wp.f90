...
! introducing collisional effects as part of her Masters and PhD research,
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine Output(WriteChoice)
    ...
    case("IL ")
       ...
       open(1,FILE= "IL1D.wt")
       do k=1,nqz
          write(1,*) -VQz(nqz+1-k),ILm(1,nqz+1-k)
       end do
       do k= 1,nqz
    ...
    case("GcL")
       ...
       do i= 1,nqcd
          write(1,*) VQQ(i),GcollL1D(i),GcollL1D(i)/Geff
       end do
       close(1)
...
!      end subroutine MIDINF 
     ...
     subroutine Coll_Damping
       ...
       case("Yes")
          ...
          do i= 1,nqcd
             ...
             Aux1_Gcoll(4)= Zlq
             Aux1_Gcoll(5)= Zsq   
             !! L waves in the standard approximation !!
             call DQsimp(Aux_GcollL,1.E-4_wp,0.28E0_wp,Res1L)
             call DQsimpb(Aux_GcollL,0.28E0_wp,1.E30_wp,Res2L)  !! Ver isso com atencao !!
             Res= Res1L+Res2L
             ...
             Res= Res1S+Res2S
             GcollS1D(i)= -Zlq*Zsq*(16._wp*sqrt(Pi)) * (Q*AA/2._wp) &
                  * (VRNeNs(m)/VRTeTs(m))**(2.5)*Geff * Res
     ...
     real(wp) function Aux_GcollL(Qp)
       ...
       !    if(Qp > 2.e-4_wp) then
             Qp2= Qp**2
             Zeta= cmplx(sigma*Zlq/Qp/sqrt(VRTeTs(m)),0._wp)
